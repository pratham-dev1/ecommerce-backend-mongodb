...
router.post("/create-invoice-pdf", async (req, res) => {
  try {
    ...
    pdf
      .create(pdfTemplate(order.user.name, orderDetails, order.totalPrice , order.createdOn), {
        childProcessOptions: {
          env: {
      ...
      .toFile(
        path.join(__dirname, "../", "billing", `Invoice-${orderId}.pdf`),
        (err) => {
          if (err) {
